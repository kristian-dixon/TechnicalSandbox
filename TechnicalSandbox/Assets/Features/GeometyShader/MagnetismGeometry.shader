Shader "Geometry/Magnetism"
{
    Properties
    {
        ...
        _Target("_Target", Vector) = (0,0,0,1)
        ...
        SubShader
    {
        ...
        Pass
        {
            ...
            void geom(triangle v2g IN[3], inout TriangleStream<g2f> triStream)
            {
                ...
                float3 targetDir = normalize((_Target - mul(unity_ObjectToWorld, float4(0, 0, 0, 1))).xyz);

                float mul = max(dot(normal, targetDir.xyz), 0);

                mul = pow(mul, _Factor);

                float4 vtxCentre = ((IN[0].vertex + IN[1].vertex + IN[2].vertex) / 3.0f) + float4(targetDir,0) * _Extrusion * mul;
