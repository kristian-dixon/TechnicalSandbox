    ...
    SubShader
        {
            ...
            fixed4 Image(float2 uv) {
                ...
                float vert = sin(uv.y * 70.) * .015;
                float hor  = sin(uv.x * 20.) * .025;

                //Was lazy here-- couldn't be bothered to return a fixed2 
                return fixed4(rainDistort + float2(vert,hor), 0, 0);//fixed4(uv.x, uv.y, 0, 1);
            }
            ...
            void surf(Input IN, inout SurfaceOutputStandard o)
            {
                ...
                fixed4 dy = Image(frac(IN.worldPos.xz * scale));//tex2D(_MainTex, IN.worldPos.xz) * _Color;
                ...
                //TODO:: For proper refraction do some fancy normal mapping stuff
                half4 bgcolor = tex2Dproj(_BackgroundTexture, IN.grabPos + c);
                c = bgcolor;
