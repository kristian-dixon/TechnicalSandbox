...
Shader "TriplanarMapping/ImprovedDrippyTexture"
{
    Properties
    {
    ...
    SubShader
        {
            ...
            void surf(Input IN, inout SurfaceOutputStandard o)
            {
                ...
                float3 aNormal = pow(abs(normal), 5);;

                float2 dxCoords = frac(float2(wp.z * sign(normal.x), wp.y) * scale) ;
                fixed4 dx = Image(dxCoords); 

                float2 dyCoords = frac(float2(wp.x * sign(normal.y),  -sign(normal.z) * wp.z) * scale) ;
                fixed4 dy = Image(dyCoords);
                float2 dzCoords = frac(float2(-wp.x * sign(normal.z), wp.y) * scale) ;
                fixed4 dz = Image(frac(dzCoords));



                //fixed4 c = aNormal.x * dx;// + aNormal.y * dy + aNormal.z * dz;
                //fixed4 c = aNormal.y * dy;// + aNormal.z * dz;
                //fixed4 c = aNormal.z * dz;
                fixed4 c = aNormal.x * dx + aNormal.y * dy + aNormal.z * dz;

                o.Albedo = tex2D(_MainTex, IN.uv_MainTex) * _Color *  (1.0 - length(c));;
                ...
                float l = length(c);
                o.Metallic = l + _Metallic;
                o.Smoothness = l + _Glossiness;
